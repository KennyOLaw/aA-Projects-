...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    toys
    WHERE 
    toys.price = 9 and toys.name != 'Shiny Mouse'
    ORDER BY toys.name ASC;
  SQL
end
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      other_cats.name AS cats,
      jets_toys.name AS toys
    FROM
      toys other_cats_toys
    JOIN 
      toys jets_toys ON jets_toys.id = other_cats_toys.id
    JOIN 
      cattoys jet_ct ON jets_toys.id = jet_ct.toy_id
    JOIN 
      cattoys other_ct ON other_ct.toy_id = other_cats_toys.id
    JOIN 
      cats other_cats ON other_cats.id = other_ct.cat_id
    JOIN
      cats jet ON jet.id = jet_ct.cat_id
    WHERE 
      jet.name = 'Jet' AND 
      jets_toys.id = other_cats_toys.id AND 
      other_cats.name != 'Jet'
    ORDER BY 
      other_cats.name ASC;
  SQL
...
def toys_that_jet_owns_sub
  ...
  execute(<<-SQL)
    ...
    FROM
      cats AS other_cats
    JOIN 
      cattoys ON cattoys.cat_id = other_cats.id
    JOIN
      toys ON cattoys.toy_id = toys.id
    WHERE
      toys.id IN (
                ...
                LEFT OUTER JOIN
                  cats ON cattoys.cat_id = cats.id
                WHERE
                  cats.name = 'Jet') AND other_cats.name != 'Jet'
    ORDER BY 
      other_cats.name ASC;
  SQL
end